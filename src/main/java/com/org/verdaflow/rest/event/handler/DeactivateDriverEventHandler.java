...
@Component
public class DeactivateDriverEventHandler implements ApplicationListener<DeactivateDriverEvent> {
	public static final Logger log = LoggerFactory.getLogger(DeactivateDriverEventHandler.class);
	...
	@Override
	public void onApplicationEvent(DeactivateDriverEvent event) {
		log.info("onApplicationEvent");
		String subject = StringConst.SUBJECT_ORG_DEACTIVATE_DRIVER;

		Map<String, Object> model = new HashMap<>();
		model.put(StringConst.NAME, event.getName());

		String msg = VelocityEngineUtils.mergeTemplateIntoString(velocityEngine,
				"templates/emails/DeactivateDriver.vsl", StringConst.UTF_8, model);
		...
		try {
			...
			log.error(new StringBuilder(StringConst.ERROR_SENDING_DEACTIVATE_DRIVER_MAIL_TO).append(event.getEmail())
