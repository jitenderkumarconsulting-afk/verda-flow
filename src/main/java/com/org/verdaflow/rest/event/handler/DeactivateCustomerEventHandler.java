...
@Component
public class DeactivateCustomerEventHandler implements ApplicationListener<DeactivateCustomerEvent> {
	public static final Logger log = LoggerFactory.getLogger(DeactivateCustomerEventHandler.class);
	...
	@Override
	public void onApplicationEvent(DeactivateCustomerEvent event) {
		log.info("onApplicationEvent");
		String subject = StringConst.SUBJECT_ORG_DEACTIVATE_CUSTOMER;
		...
		String msg = VelocityEngineUtils.mergeTemplateIntoString(velocityEngine,
				"templates/emails/DeactivateCustomer.vsl", StringConst.UTF_8, model);
		...
		try {
			...
			log.error(new StringBuilder(StringConst.ERROR_SENDING_DEACTIVATE_CUSTOMER_MAIL_TO).append(event.getEmail())
